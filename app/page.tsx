    ...
    <main className="relative min-h-screen bg-[var(--bg)] text-[var(--fg)] overflow-hidden flex flex-col items-center justify-center px-6">
      ...
      >
        ...
        >
          <Link
            href="/cadastro"
            className="border border-[var(--border)] hover:bg-[var(--muted)] px-6 py-3 rounded-2xl font-semibold transition backdrop-blur"
          >
            Criar ficha
          </Link>
      ...
      >
        {[
          ...
          >
            ...
            <p className="text-[var(--fg)]/70">{item.desc}</p>
          </motion.div>
        ))}
      </motion.section>
      ...
      <footer className="relative mt-24 text-[var(--fg)]/50 text-sm z-10">
        © {new Date().getFullYear()} Dark Roses • Feito para leitores intensos
      </footer>
    </main>
  );
}
